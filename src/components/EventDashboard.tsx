    ...
    const handleCreateEvent = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (newEvent.coverImage) {
                const coverImageKey = `events/shared/${eventId}/cover-${newEvent.coverImage.name}`;
        ...
        <div className="relative min-h-screen bg-red-100">
            <div className="relative z-10 container mx-auto px-4 py-8">
                <div className="mb-8 flex justify-between items-center">
                    <h1 className="text-3xl font-bold text-red-900">Event Dashboard</h1>
                    <button
                        onClick={() => setIsModalOpen(true)}
                        className="flex items-center bg-red-100 text-red-700 py-2 px-4 rounded-lg hover:bg-secondary transition-colors duration-200"
                    >
                    ...
                    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                        <div className="bg-max-w-2xl mx-auto bg-black p-8 rounded-lg shadow-md border-2 border-aquamarine rounded-lg p-8 max-w-md w-full">
                            <div className="flex justify-between items-center mb-6">
                                <h2 className="text-2xl font-bold text-red-400">Create New Event</h2>
                                <button
                            ...
                            <form onSubmit={handleCreateEvent} className="space-y-4">
                                ...
                                <div className="mb-4">
                                    <label className="block text-red-200 mb-2" htmlFor="coverImage">
                                        Cover Image
                                    </label>
                                    <input
                                        type="file"
                                        ...
                                        className="w-full text-red-200 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-red-50 file:text-red-700 hover:file:bg-red-100"
                                    />
                                </div>
                                <div>
                                    <label className="block text-red-200 mb-2" htmlFor="eventName">
                                        Event Name
                                    </label>
                                    <input
                                        type="text"
                                        ...
                                        className="w-full border border-red-300 rounded-lg px-4 py-2 focus:outline-none focus:border-primary"
                                        required
                                    />
                                </div>
                                <div>
                                    <label className="block text-red-200 mb-2" htmlFor="eventDate">
                                        Event Date
                                    </label>
                                    <input
                                        type="date"
                                ...
                                <button
                                    type="submit"
                                    disabled={isLoading}
                                    className="w-full bg-red-300 text-black py-2 px-4 rounded-lg hover:bg-secondary transition-colors duration-200 disabled:opacity-50"
                                >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mt-8">
                    ...
                    <div onClick={() => setShowAllEvents(!showAllEvents)} className="cursor-pointer">
                        <StatsCard
                            icon={<Image className="w-6 h-6 text-red-900" />}
                            title="Total Events"
                    ...
                    <StatsCard
                        icon={<Camera className="w-6 h-6 text-red-900" />}
                        title="Total Photos"
                        count={stats.photoCount}
                        bgColor="bg-red-400"
                    />
                    <StatsCard
                        icon={<Video className="w-6 h-6 text-red-900" />}
                        title="Total Videos"
                ...
                {showAllEvents && (
                    <div className="mt-8">
                        <h2 className="text-2xl font-bold text-red-900 mb-6">All Events</h2>
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {Array.isArray(events) && events.map((event) => (
                                <div key={event.id} className="bg-red-100 rounded-lg shadow-md overflow-hidden">
                                    <div className="w-full h-48 bg-black flex items-center justify-center">
                                        {event.coverImage ? (
                                            <img src={event.coverImage} alt={event.name} className="w-full h-full object-cover" />
                                        ) : (
                                            <Camera className="w-12 h-12 text-red-700" />
                                        )}
                                    </div>
                                    <div className="p-4">
                                        ...
                                        <div className="flex justify-between items-center">
                                            ...
                                            <div className="mt-4 flex justify-end space-x-4">
                                                <Link
                                                    ...
                                                    className="bg-red-300 text-black px-4 py-2 rounded-lg hover:bg-red-600 transition-colors duration-200"
                                                >
                                                    View Event
                                                </Link>
                                                <button
                                                    onClick={() => setDeleteConfirmation({isOpen: true, eventId: event.id})}
                                                    className="bg-red-500 text-red px-4 py-2 rounded-lg hover:bg-red-600 transition-colors duration-200 flex items-center"
                                                >
